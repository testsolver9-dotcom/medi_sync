...
async def run_test():
    ...
    try:
        ...
        # Select 'Hospital' as practice location and click Continue
        frame = context.pages[-1]
        ...
        # Input a patient ID and click Request Consent to send access consent request
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/input').nth(0)
        ...
        # Verify consent status visibility and access history for the patient
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/nav/a[3]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Verify consent status visibility on Dashboard or Patient Access Consent page
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div/nav/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click 'View Details' on the most recent patient record to check for consent status visibility
        frame = context.pages[-1]
